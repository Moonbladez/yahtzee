...
class Rule {
	...
	//how many times a certain number appears in dice hand
	count(dice, val) {
...
class FullHouse extends Rule {
	evalRoll = (dice) => {
		const frequencey = this.freq(dice);
		return frequencey.includes(2) && frequencey.includes(3) ? this.score : 0;
	};
}
...
class SmallStraight extends Rule {
	evalRoll = (dice) => {
		...
		//345 and either 2 or 6
		if (
			diceSet.has(3) &&
			diceSet.has(4) &&
			diceSet.has(5) &&
			(diceSet.has(2) || diceSet.has(6))
		)
			return this.score;
		return 0;
	};
